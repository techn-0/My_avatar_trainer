...
const LoginToggle = ({ onClose }) => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://15.165.191.221:3002/auth/signin", {
        method: "POST",
  ...
  const handleSignUpSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://15.165.191.221:3002/auth/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSocialLogin = (provider) => {
    const baseURL = "http://15.165.191.221:3002/socauth"; // 소셜 로그인 엔드포인트의 베이스 URL
